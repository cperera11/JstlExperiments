...
<html>
    ...
    <body>
        ...
        <form name="form1" id="form1" method="POST" action="ColorController">
            ...
            <c:set var="sColor" value="${secretColor}"/>
            <div id="hints"  style="display:none;" > 
                The secret color has <b>${fn:length(sColor)}</b> letters <br>
                The first letter of the color is <b>${fn:substring(sColor, 0,1)}</b>
            </div>

            <c:choose>
                <c:when test="${color == secretColor}">
                    <p>${winMsg} <b>${sColor}</b></p>
                </c:when>
                <c:otherwise>
                    <p> ${tryMsg}</p>
                </c:otherwise>
